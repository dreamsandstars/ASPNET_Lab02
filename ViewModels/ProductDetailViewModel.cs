...
public class ProductDetailViewModel
{
    ...
    public string ReorderSuggestion
    {
        get
        {
            if (Quantity <= 0) return "Cần nhập hàng ngay vì sản phẩm đã hết.";
            if (Quantity <= MinStock) return $"Nên nhập thêm. Tồn kho hiện tại chỉ còn {Quantity}, mức tối thiểu là {MinStock}.";
            if (Quantity >= 20) return "Tồn kho đang cao, chưa cần nhập thêm.";
